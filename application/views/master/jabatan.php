					...
					<table id="table"
						...
						<thead class="thead-light">
							<tr>
								...
								<th data-field="no" data-formatter="nomerFormatter" data-width="10" data-width-unit="%">No</th>
								<th data-field="nama_jabatan" data-sortable="true" data-width="80" data-width-unit="%" >Jabatan</th>
			...
			<div class="modal-body p-0">
				<div class="card bg-secondary border-0 mb-0">
					<div class="card-header bg-transparent pb-5">
						<div class="text-muted text-center mt-2 mb-3">Input Data Jabatan Baru</div>
					</div>
		...
		if(row){
			Swal.fire({
				...
				if (result.value) {
					...
					$.post('jabatan/delete',{id:data},function(result){
						if (result.errorMsg){
							Toast.fire({
							type: 'error',
	...
	function aktif(){
		...
		if(row){
			Swal.fire({
				...
				if (result.value) {
					$.post('jabatan/aktif',{id:row._id},function(result){
						if (result.errorMsg){
							Toast.fire({
							type: 'error',
							title: ''+result.errorMsg+'.'
